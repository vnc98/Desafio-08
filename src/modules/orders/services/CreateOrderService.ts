...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductWithnotQuantity = products.filter(
      product =>
        productsExists.filter(p => p.id === product.id)[0].quantity <=
        product.quantity,
    );

    if (findProductWithnotQuantity.length) {
      throw new AppError(
        `The quantity ${findProductWithnotQuantity[0].quantity} is not available`,
      );
    }
